...
from scripts.inference.predict_freight import predict_freight_cost
from scripts.inference.predict_invoice_flag import predict_invoice_flag
...
### AT-Driven Freight Cost Prediction & Invoice Risk Flagging

This internal analytics portal leverages machine learning to 
- **Forecast freight costs accurate**
- **Detect risky or abnormal vendor invoices**
- **Reduce financial lekage and manual workload**
"""
)
...
if selected_model == "Freight Cost Prediction":
    ...
    if submit_freight:
        ...
        st.success("Prediction completed successfiully")

        st.metric(
            label="🏷️ Estimated Freight Cost",
            value=f"${prediction[0]:,.2f}"
        )

# Invoice Flag Prediction
else:
    st.subheader("🧾 Invoice Manual Aprroval Prediction")

    st.markdown("""
                **Objective:**
                Predict whether a vendor should be **flagged for manual approval**
    ...
    if submit_flag:
        ...
        if is_flagged:
            st.error("🚨 Invoice required **Manual Approval**")
        else:
